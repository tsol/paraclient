...
import TradeEntries from './overlays/TradeEntries.vue';
...
export default {
  ...
  data: () => ({
    overlays: [ValueBars, CandleDebug, ATR, MACD, BBANDS, TradeEntries],
    colors: {
      colorBack: '#fff',
      colorGrid: '#eee',
      colorText: '#333',
  ...
  computed: {
    ...
    candleDebugData() {
      return this.cdebug.map((cdb) => {
        const candle = this.candles.find((c) => c.openTime === cdb.time);
        if (!candle) {
