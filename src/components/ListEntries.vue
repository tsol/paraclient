...
-->
  ...
  gain: <b>{{ this.sumSelectedGain }}%</b>, win/loose: <b>{{ this.winLooseRatio }}</b> </div>
...
export default {
  ...
  watch: {
    searchInput: debounce(
      function post(v) { this.filter.tags = v.split(/,\s*/); this.loading = false; },
      700,
      function pre() { this.loading = true; },
    ),
